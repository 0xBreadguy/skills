...
Use the current official Realtime API method:

| Method | Status |
|--------|--------|
| `realtime_sendRawTransaction` | Supported |

Historical drafts and older skill material may mention
`eth_sendRawTransactionSync`. Do not use that name in new code unless the
target RPC endpoint explicitly documents support for it.
...
| realtime_sendRawTransaction | 200-450ms | **lower-latency receipt return without polling** |
...
const ws = new WebSocket('wss://mainnet.megaeth.com/ws');
ws.send(JSON.stringify({ method: 'realtime_sendRawTransaction', ... }));
```
...
const receipt = await rpc('realtime_sendRawTransaction', [signedTxs[0]]);
```
